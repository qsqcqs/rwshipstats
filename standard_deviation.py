...
for w in pss:
    ...
    svals[int((w[0])[0])].append(int(w[2]))
    svals[int((w[0])[2])].append(int(w[2]))
    svals[int((w[1])[0])].append(int(w[3]))
    svals[int((w[1])[2])].append(int(w[3]))
...
while x<10:
    k=0
    for s in svals[x]:
        k=k+(s-smag[x])*(s-smag[x])
    k=sqrt(k/len(svals[x]))
    smag[x]=k
    x=x+1
